...
class WC_Conditional_Content_Display {
    ...
    public function __construct() {
        add_action( 'init', [ $this, 'load_locations' ], 0 );
        add_action( 'template_redirect', [ $this, 'bind_display' ], -1 );
    }

    public function load_locations() {
        $this->locations = apply_filters( 'wc_conditional_content_get_locations', [], 0 );
    }
        ...
        if ( $contents && count( $contents ) ) {
            ...
            foreach ( $contents as $content ) {
                ...
                if ( $display ) {
                    ...
                    if ( $custom_hook ) {
                        $hook = apply_filters(
                            'woocommerce_conditional_content_get_output_hook',
                            [
                                'action'        => $custom_hook,
                        ...
                        if ( $hook && isset( $hook['action'] ) && isset( $hook['priority'] ) ) {
                            ...
                            add_action( $hook['action'], [ $this, $tag ], $hook['priority'], $accepted_args );
                        }
                ...
                if ( $display ) {
                    ...
                    if ( $echo ) {
                        wc_get_template(
                            ...
                            WC_Conditional_Content::plugin_path() . '/templates/'
                        );
                    }
        ...
        if ( isset( self::$contents[ $name ] ) ) {
            foreach ( self::$contents[ $name ] as $content ) {
                if ( $this->match_groups( $content, $arguments ) ) {
                    wc_get_template( 'contentblock.php', [ 'content' => $content ], 'woocommerce-conditional-content', WC_Conditional_Content::plugin_path() . '/templates/' );
                }
            }
        }
